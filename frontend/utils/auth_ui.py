...
def render_auth_sidebar() -> None:
    ...
    if user is None:
        ...
        st.markdown(
            ...
            f"{'✅ ' + str(remaining) + ' remaining today' if remaining > 0 else '⛔ Quota reached — resets in <24h'}"
            f"</div></div>",
            unsafe_allow_html=True,
        )

        # Logout
        if st.button("↩ Logout", use_container_width=True, key="sb_logout_btn"):
            logout()
