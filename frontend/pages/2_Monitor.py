...
[data-testid="stAppViewContainer"]{background:linear-gradient(135deg,#060b18,#0a1224,#0f0a1e);}
[data-testid="stSidebar"]{background:linear-gradient(180deg,#060c1a,#0d1525);border-right:1px solid #1e293b;}
.sec-hdr{display:flex;align-items:center;gap:.7rem;margin:1.5rem 0 1rem;padding-bottom:.7rem;border-bottom:1px solid rgba(99,102,241,.25);}
.sec-title{font-size:1.2rem;font-weight:600;color:#e2e8f0;}
/* Agent cards */
.a-card{background:rgba(15,23,42,.9);border:1px solid rgba(30,41,59,.9);border-radius:12px;padding:1rem;transition:all .25s;}
.a-idle   {border-left:3px solid #334155;}
.a-running{border-left:3px solid #6366f1;box-shadow:0 0 18px rgba(99,102,241,.25);}
...
.s-running{background:rgba(99,102,241,.2);color:#818cf8;animation:pulse 1.5s infinite;}
...
/* Hardware card */
.hw-card{background:rgba(15,23,42,.9);border:1px solid rgba(99,102,241,.2);border-radius:16px;padding:1.5rem;margin-bottom:1rem;}
...
.m-box{background:rgba(8,15,30,.8);border:1px solid rgba(30,41,59,.8);border-radius:10px;padding:1rem;text-align:center;}
...
.m-util{color:#818cf8;} .m-mem{color:#38bdf8;} .m-temp{color:#fb923c;} .m-pow{color:#4ade80;}
/* Backend banner */
...
/* Scholarship table */
.sr{display:grid;grid-template-columns:2fr 1.5fr .8fr .8fr .8fr;gap:.5rem;align-items:center;padding:.7rem 1rem;border-radius:8px;margin-bottom:.3rem;background:rgba(15,23,42,.7);border:1px solid rgba(30,41,59,.6);font-size:.85rem;transition:all .2s;}
.sr:hover{border-color:rgba(99,102,241,.35);background:rgba(99,102,241,.05);}
.sh{font-size:.72rem;font-weight:600;color:#475569;text-transform:uppercase;letter-spacing:.08em;padding:.4rem 1rem;}
.sn{color:#cbd5e1;font-weight:500;} .si{color:#64748b;} .sc{color:#38bdf8;}
...
if not disp:
    ...
    for s in disp[:100]:
        ...
        st.markdown(f'<div class="sr"><div class="sn">{name_html}</div><div class="si">{inst}</div>'
                    f'<div class="sc">{country}</div><div class="{sc_cls}">{sc_lbl}</div>'
